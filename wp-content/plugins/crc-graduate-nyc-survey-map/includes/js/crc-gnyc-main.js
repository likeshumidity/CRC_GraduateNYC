...
// map setup
    ...
    GNYC.updateCBOLocations = function(data) {
        d3.selectAll('.cbo-location').transition()
            .duration(750)
...
// listing setup
    ...
    GNYC.listingItemHTML = function(item) {
        ...
        if (item.program_description.length > 0 && item.services2.length > 0) {
            htmlSnippet += '<em class="viewMore"><i class="fa fa-plus readmoreless"></i> View program details</em>';
        } else {
            htmlSnippet += '<em class="viewMore"> No details provided</em>';
        }
